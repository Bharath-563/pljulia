...
# 1. Install build dependencies + patchelf (to fix the security flag)
...
# 2. Install Julia and surgically REMOVE the 'executable stack' requirement
RUN set -eux; \
...
# 3. Build with Embedded Paths (RPATH)
ENV USE_PGXS=1
...
# 4. Run tests - The patchelf fix above ensures this server starts!
RUN set -eux; \
